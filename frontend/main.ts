// Type definitions
type Pattern = 'adapter' | 'decorator' | 'facade' | 'flyweight' | 'proxy' | 'bridge' | 'composite';
...
let currentCafe: any = null;
let sistemaPedido: any = null;
let sistemaParticulas: any = null;
let controleTV: any = null;
let controleRadio: any = null;
let estruturaArquivos: any = null;

function initializePatternState(pattern: Pattern) {
    switch (pattern) {
        case 'adapter':
            audioPlayer = new (window as any).AudioPlayer();
            break;
        case 'facade':
            sistemaPedido = new (window as any).SistemaPedidoFacade();
            break;
        case 'flyweight':
            const fabrica = new (window as any).FabricaParticulas();
            sistemaParticulas = new (window as any).SistemaParticulas(fabrica);
            break;
        case 'bridge':
            const TV = (window as any).TV;
            const RadioFM = (window as any).RadioFM;
            const ControleRemotoAvancado = (window as any).ControleRemotoAvancado;
            ...
            break;
        case 'composite':
            estruturaArquivos = (window as any).criarEstruturaExemplo();
            break;
    }
}
...
function updateDemo() {
    ...
    if (!demoContainer || !descriptionElement || !componentsElement || !controlsElement) return;
...
function setupAdapterDemo(description: HTMLElement, components: HTMLElement, controls: HTMLElement) {
...
function setupDecoratorDemo(description: HTMLElement, components: HTMLElement, controls: HTMLElement) {
    description.innerHTML = `
...
function setupFacadeDemo(description: HTMLElement, components: HTMLElement, controls: HTMLElement) {
    description.innerHTML = `
...
function setupFlyweightDemo(description: HTMLElement, components: HTMLElement, controls: HTMLElement) {
    description.innerHTML = `
...
function setupProxyDemo(description: HTMLElement, components: HTMLElement, controls: HTMLElement) {
    description.innerHTML = `
        <h2>Proxy Pattern</h2>
...
function setupBridgeDemo(description: HTMLElement, components: HTMLElement, controls: HTMLElement) {
    description.innerHTML = `
...
function setupCompositeDemo(description: HTMLElement, components: HTMLElement, controls: HTMLElement) {
    description.innerHTML = `
...
function playMedia(filename: string) {
...
function updatePlayerDisplay(result?: string, adapter?: string, description?: string) {
    const componentsElement = document.getElementById('components');
...
function createBaseCoffee(type: string) {
    const CafeClass = (window as any)[type];
    if (CafeClass) {
...
function addDecorator(type: string) {
    if (!currentCafe) {
        alert('Por favor, escolha primeiro um café base!');
        return;
    }
    const DecoratorClass = (window as any)[type];
    if (DecoratorClass) {
...
function realizarPedidoCompleto() {
    if (!sistemaPedido) return;
    
    const result = sistemaPedido.realizarPedido(
        "Notebook",
        3500,
        "cliente@email.com",
        "Rua Principal, 123",
...
function criarParticulas() {
    ...
    for (let i = 0; i < 5; i++) {
        sistemaParticulas.criarParticula(
            Math.random() > 0.5 ? "vermelho" : "azul",
            Math.random() > 0.5 ? "fogo" : "água",
            Math.random() > 0.5 ? 2 : 1,
            Math.random() * 100,
            Math.random() * 100,
            Math.random() * 5
...
function carregarImagemLazy() {
    const ProxyImagem = (window as any).ProxyImagem;
...
function carregarImagemCache() {
    const ProxyImagemComCache = (window as any).ProxyImagemComCache;
...
function acessarImagemProtegida(nivel: number) {
    const ProxyImagemProtegida = (window as any).ProxyImagemProtegida;
    const proxy = new ProxyImagemProtegida("foto_secreta.jpg", nivel);
    proxy.exibir();
    atualizarProxyDisplay("Proteção");
}

function atualizarProxyDisplay(tipo: string) {
    const componentsElement = document.getElementById('components');
    if (componentsElement) {
        componentsElement.innerHTML += `
            <div class="component">
...
function controlarTV(acao: string) {
    if (!controleTV) return;
...
function controlarRadio(acao: string) {
    if (!controleRadio) return;
...
function atualizarDispositivoDisplay(dispositivo: string) {
    const componentsElement = document.getElementById('components');
    if (componentsElement) {
        componentsElement.innerHTML += `
            <div class="component">
...
function adicionarArquivo() {
    ...
    if (nome && tamanho) {
        const Arquivo = (window as any).Arquivo;
...
function adicionarDiretorio() {
    ...
    if (nome) {
        const Diretorio = (window as any).Diretorio;
...
(window as any).setPattern = setPattern;
(window as any).playMedia = playMedia;
(window as any).createBaseCoffee = createBaseCoffee;
(window as any).addDecorator = addDecorator;
(window as any).realizarPedidoCompleto = realizarPedidoCompleto;
(window as any).criarParticulas = criarParticulas;
(window as any).mostrarEstatisticas = mostrarEstatisticas;
(window as any).carregarImagemLazy = carregarImagemLazy;
(window as any).carregarImagemCache = carregarImagemCache;
(window as any).acessarImagemProtegida = acessarImagemProtegida;
(window as any).controlarTV = controlarTV;
(window as any).controlarRadio = controlarRadio;
(window as any).mostrarEstrutura = mostrarEstrutura;
(window as any).adicionarArquivo = adicionarArquivo;
(window as any).adicionarDiretorio = adicionarDiretorio;
(window as any).calcularTamanho = calcularTamanho;
